    ...
    def get_historical_data(self, ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            if not self.table_exists(table_name):
                logging.info(f"Table {table_name} does not exist. Creating and populating from yfinance...")
            ...
            if max_date is None or min_date is None:
                ...
                logging.info(f"Table {table_name} is empty. Populating from yfinance...")
                df = self._fetch_yfinance_data(ticker, start_date, end_date)
                if df is not None:
                    self.store_dataframe(df, table_name)
                    return df
            ...
            if days_difference > 10:
                ...
                if not rows:
                    logging.info("No data found in database for specified date range. Fetching from yfinance...")
                    df = self._fetch_yfinance_data(ticker, start_date, end_date)
                    if df is not None:
                        self.store_dataframe(df, table_name)
                    return df
